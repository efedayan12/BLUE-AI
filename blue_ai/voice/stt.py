...
class STTEngine:
    ...
    def __init__(self, model_size: str = "small", device: str = "cpu", compute_type: str = "int8"):
        ...
        small model Türkçe için önerilir (tiny çok hatalı yapar).
        ...
        self.silence_duration = 0.9     # Kaç saniye sessizlik = konuşma bitti
    ...
    def _fallback_to_cpu(self, model_size: str = "") -> bool:
        """CUDA başarısızsa CPU + small modele düş."""
        if not WHISPER_AVAILABLE:
            return False
        size = model_size or ("small" if self.model_size in ("medium", "large") else self.model_size)
        try:
            self.device = "cpu"
            self.compute_type = "int8"
            self.model_size = size
            self.model = WhisperModel(size, device="cpu", compute_type="int8",
                                      num_workers=1, cpu_threads=8)
            print(f"[STT] CPU fallback: {size}/cpu/int8")
            return True
        except Exception as e:
            print(f"[STT] CPU fallback basarisiz: {e}")
            self.model = None
            return False

    def load_model(self) -> bool:
        """Whisper modelini yükle — CUDA→small-CUDA→CPU zinciri."""
        if not WHISPER_AVAILABLE:
            print("[STT] faster-whisper kurulu degil.")
            return False
        if self.model is not None:
            return True

        # 1. İstenen ayarlarla dene (örn. cuda+medium)
        try:
            ...
            print(f"[STT] Whisper hazir: {self.model_size}/{self.device}/{self.compute_type}")
            return True
        except Exception as e:
            print(f"[STT] Yukleme hatasi ({self.model_size}/{self.device}): {e}")

        # 2. CUDA+medium başarısızsa CUDA+small dene (daha az VRAM)
        if self.device == "cuda" and self.model_size != "small":
            try:
                self.model_size = "small"
                self.model = WhisperModel("small", device="cuda",
                                          compute_type=self.compute_type,
                                          num_workers=1, cpu_threads=8)
                print("[STT] Fallback: small/cuda")
                return True
            except Exception as e2:
                print(f"[STT] CUDA small da basarisiz: {e2}")

        # 3. Son çare: CPU + small
        return self._fallback_to_cpu()

    def _normalize_audio(self, audio_file: str) -> str:
        """Ses dosyasını normalize ve filtrele — Whisper doğruluğunu artırır."""
        ...
        try:
            ...
            if peak < 0.001:
                return audio_file  # tamamen sessiz, dokunma

            # Agresif normalize: peak'i 0.9'a çek (zayıf mikrofon sesini güçlendir)
            if peak < 0.9:
                data = data * (0.9 / peak)
                # Clipping önle
    ...
    def transcribe_file(self, audio_file: str) -> str:
        ...
        kwargs = dict(
            language="tr",           # Dil tespitini atla, direkt Türkçe mod
            task="transcribe",       # Çeviri değil, transkripsiyon
            beam_size=5,             # 5 alternatif yol — en iyi seç
            best_of=5,
            temperature=0.0,         # Deterministik — en güvenilir
            condition_on_previous_text=False,
            no_speech_threshold=0.35,           # Düşük eşik = sessizliği konuşma sanmaz
            compression_ratio_threshold=2.4,
            log_prob_threshold=-1.0,            # Düşük olasılıklı transkriptleri reddet
            # Türkçe kelime haznesini aktive et — yanlış İngilizce algılamayı önler
            initial_prompt="Merhaba. Türkçe bilgisayar asistanı. Sistem durumu, ram, cpu, disk, uygulama aç, hava durumu.",
            vad_filter=True,
            vad_parameters=dict(
                min_silence_duration_ms=200,
                speech_pad_ms=400,   # Kelimelerin başı/sonu kesilmesin
                threshold=0.4,       # Düşük eşik = zayıf sesi de konuşma say
            ),
        )
        ...
        try:
            with sd.RawInputStream(
                ...
                while time.time() - start_time < max_duration:
                    ...
                    # İlk 0.8s gürültü tabanını öğren (0.5→0.8: daha stabil ölçüm)
                    if time.time() - start_time < 0.8:
                        noise_samples.append(rms)
                        if noise_samples:
                            # 1.8→1.5: daha düşük multiplier = zayıf sesi daha iyi yakala
                            noise_floor = max(self.silence_threshold,
                                              float(np.mean(noise_samples)) * 1.5)
                        continue
                    ...
                    # 6 saniye içinde hiç ses yoksa pes et (5→6: daha sabırlı)
...
def get_stt(engine: str = "") -> "STTEngine | VoskSTTEngine":
    ...
    try:
        ...
        if os.path.exists(cfg_path):
            ...
            try:
                silence_duration = float(voice_cfg.get("silence_duration", silence_duration))
    ...
    # Config'den okunan sessizlik süresini uygula
    _stt.silence_duration = silence_duration
