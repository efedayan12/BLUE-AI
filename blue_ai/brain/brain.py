...
class Brain:
    ...
    _FAST_PATH_INTENTS = frozenset({
        ...
        "analyze_screen", "click_on_screen", "type_on_screen",
        "press_keys", "scroll_screen", "watch_screen",
    })
    ...
    _NOT_A_VALUE = frozenset({
        ...
        "adim", "ismim", "adi", "ismi", "adin", "ismin",
        # Komut/bağlaç kelimeleri (isim olarak yakalanmamalı)
        "olsun", "olacak", "olarak", "diye", "yap", "yapsin", "kaydet", "hatirla",
        "desin", "desinler", "diyebilirsin", "cagir", "bil", "benim", "senin",
        "artik", "bundan", "sonra", "lutfen",
    })
    ...
    _FACT_PATTERNS: list[tuple[str, str]] = [
        # İsim — doğal dil kalıpları (esnek)
        # "ismin hasan olsun", "adın hasan olsun", "adı hasan olsun"
        (r"\b(?:ismin|adin|adi|ismi)\s+(\w{2,20})\s+ol", "isim"),
        # "bana hasan de", "bana hasan diye seslen/hitap et"
        (r"\bbana\s+(\w{2,20})\s+(?:de|di|diye|desinler|seslen|hitap)", "isim"),
        # "beni hasan olarak kaydet", "ismimi hasan yap", "adımı hasan olarak bil"
        (r"\b(?:beni|ismimi|adimi)\s+(\w{2,20})\b", "isim"),
        # "benim adım X", "adım X", "ismim X"
        (r"benim adim (\w{2,20})", "isim"),
        (r"^adim (\w{2,20})", "isim"),
        (r"benim ismim (\w{2,20})", "isim"),
        (r"^ismim (\w{2,20})", "isim"),
        # "adım hasan", "ismim hasan", "adın hasan", "ismin hasan"
        # (cümle içinde, baştaki 'benim' olmadan — soru cümleleri is_question ile elenir)
        (r"\b(?:adim|ismim|adin|ismin)\s+(\w{2,20})\b", "isim"),
    ...
    def _try_memory_operation(self, user_message: str) -> "BrainResponse | None":
        ...
        # ── "adın ne?", "ismin ne?", "adını söyle" → kayıtlı ismi söyle ─────
        # ("benim adım ne" aşağıdaki _MEMORY_QUERIES'te ayrıca ele alınır)
        if "benim" not in msg_n and (
                re.search(r"\b(adin|ismin)\s+(ne|nedir|neydi|kim)\b", msg_n)
                or re.search(r"\b(adini|ismini)\s+soyle", msg_n)
                or re.search(r"\bsenin\s+(adin|ismin)\b", msg_n)):
            value = perm.get_fact("isim")
            if value:
                return BrainResponse(
                    message=f"Adım {value}! 😊",
                    response_type="success")
            return BrainResponse(
                message="Henüz bana bir isim vermedin. Örneğin 'Adın Hasan olsun' diyebilirsin!",
                response_type="info")
        ...
        if not is_question:
            for pattern, fact_key in self._FACT_PATTERNS:
                ...
                # "adın/ismin X olsun" → asistana isim verildi (2. tekil şahıs)
                if (fact_key == "isim" and "benim" not in msg_n
                        and re.search(r"\b(adin|ismin)\b", msg_n)):
                    return BrainResponse(
                        message=f"Tamam, artık adım {display}! 😊 Kapatıp açsan bile hatırlayacağım.",
                        response_type="success")
    ...
    def _extract_fast_path_args(self, intent: str, user_message: str) -> dict:
        ...
        if intent == "close_app":
            return self._extract_app_name(user_message)
        if intent == "write_to_notepad":
            return self._extract_notepad_text(user_message)
        if intent == "click_on_screen":
            return self._extract_click_target(user_message)
        if intent == "type_on_screen":
            return {"text": self._extract_screen_type_text(user_message)}
        if intent == "press_keys":
            return {"keys": self._extract_keys(user_message)}
        if intent == "scroll_screen":
            n = self._normalize_turkish(user_message.lower())
            up = any(w in n for w in ("yukari", "yukar", "up"))
            return {"direction": "yukari" if up else "asagi"}
        if intent == "watch_screen":
            n = self._normalize_turkish(user_message.lower())
            stop = any(w in n for w in ("durdur", "birak", "kapat", "iptal", "stop"))
            return {"action": "durdur" if stop else "baslat"}
        return {}

    def _extract_notepad_text(self, message: str) -> dict:
        """'not defterine X yaz' → {'text': 'X', 'mode': 'ekle'/'yeni'}."""
        text = (message or "").strip()
        norm = self._normalize_turkish(text.lower())
        mode = "yeni" if any(w in norm for w in ("yeni not", "temiz", "bastan")) else "ekle"
        cleaned = text
        # Baştaki komut kalıpları (not defterine / notepada / deftere)
        prefixes = [
            r"^\s*(?:lütfen|lutfen)?\s*(?:yeni\s+)?not\s*defter(?:ine|ime|i|in)?\s*",
            r"^\s*(?:yeni\s+)?notepad['a-zçğıöşü]*\s*",
            r"^\s*(?:yeni\s+)?deftere\s*",
        ]
        # Sondaki komut fiilleri (yaz / ekle / not al)
        suffixes = [
            r"\s*(?:diye\s*)?(?:not\s*(?:al|et)|yaz(?:ar\s*m[ıi]s[ıi]n)?|ekle(?:r\s*m[ıi]s[ıi]n)?)\s*[\.!\?]*\s*$",
        ]
        ...
        if not cleaned:
            cleaned = text
        return {"text": cleaned, "mode": mode}

    # Bilinen uygulama adları → APP_MAP anahtarı (uzun ifadeler önce eşleşmeli)
    _APP_ALIASES: dict[str, str] = {
        "not defteri": "notepad", "not defterini": "notepad",
        "not defterimi": "notepad", "notdefteri": "notepad",
        "hesap makinesi": "calc", "hesap makinasi": "calc",
        "dosya gezgini": "explorer", "gorev yoneticisi": "task manager",
        "vs code": "vscode", "visual studio code": "vscode",
        "komut istemi": "cmd", "ekran alintisi": "snipping tool",
        "winword": "word", "word": "word", "excel": "excel",
        "powerpoint": "powerpoint", "outlook": "outlook",
        "notepad++": "notepad++", "notepad": "notepad", "defter": "notepad",
        "calc": "calc", "calculator": "calc", "hesap": "calc",
        "chrome": "chrome", "krom": "chrome", "firefox": "firefox",
        "edge": "edge", "opera": "opera", "brave": "brave",
        "paint": "paint", "explorer": "explorer", "cmd": "cmd",
        "powershell": "powershell", "vscode": "vscode", "spotify": "spotify",
        "discord": "discord", "steam": "steam", "whatsapp": "whatsapp",
        "telegram": "telegram", "vlc": "vlc", "paint3d": "paint",
        "ayarlar": "ayarlar", "settings": "settings",
    }

    def _extract_app_name(self, message: str) -> dict:
        """'word aç', 'excel'i aç', 'not defterini aç' → {'app_name': 'word'}.

        Önce bilinen uygulama adlarını (en uzun ifade önce) doğrudan arar;
        bulamazsa komut kelimelerini ve Türkçe yönelme eklerini soyarak kalanı verir.
        """
        text = (message or "").strip()
        low = text.lower()
        norm = self._normalize_turkish(low)

        # 1. Bilinen uygulama adı geçiyor mu? (uzun ifadeler önce)
        for alias in sorted(self._APP_ALIASES, key=len, reverse=True):
            alias_norm = self._normalize_turkish(alias)
            # Kelime sınırıyla ara — "word" 'a' içeren kelimelerde yanlış eşleşmesin
            if re.search(rf"(?<![a-z0-9]){re.escape(alias_norm)}(?![a-z0-9])", norm):
                return {"app_name": self._APP_ALIASES[alias]}

        # 2. Komut kelimelerini soy, kalanı uygulama adı kabul et
        cleaned = norm
        cleaned = re.sub(
            r"\b(uygulamasini|uygulamasi|programini|programi|app|adli|isimli)\b",
            " ", cleaned)
        cleaned = re.sub(
            r"\b(acar\s*misin|acabilir\s*misin|ac|baslat|calistir|getir|aciver|"
            r"acsana|acsene)\b",
            " ", cleaned)
        cleaned = re.sub(r"\b(lutfen|bana|bir|hemen|simdi|su|bu)\b", " ", cleaned)
        cleaned = cleaned.replace("'", " ").replace("’", " ")
        cleaned = re.sub(r"\s+", " ", cleaned).strip(" :,.-\"'")
        # Sonda kalan tek Türkçe yönelme ekini at ("word u" → "word")
        cleaned = re.sub(r"\s+([iuü]|yi|yu|yü|u|i)$", "", cleaned).strip()
        if not cleaned:
            cleaned = low.strip()
        return {"app_name": cleaned}

    # ── Ekran komutları: tespit + parametre çıkarımı ──────────────────────────
    def _detect_screen_intent(self, msg_norm: str) -> "str | None":
        """Ekran izleme/müdahale komutlarını sezgisel tespit (normalize edilmiş metin)."""
        words = msg_norm.split()
        has_ekran = any(w.startswith("ekran") for w in words)

        # İzlemeyi başlat/durdur
        if has_ekran and any(p in msg_norm for p in ("izle", "takip et", "canli")):
            return "watch_screen"
        if any(p in msg_norm for p in ("izlemeyi durdur", "izlemeyi birak",
                                       "izlemeyi kapat", "ekrani birak")):
            return "watch_screen"

        # Ekranı oku / analiz / ne var
        if has_ekran and any(p in msg_norm for p in (
                "ne var", "ne yaziyor", "oku", "analiz", "ne goruyorsun",
                "incele", "ne goruyor", "anlat", "ne gosteriyor")):
            return "analyze_screen"

        # Tıklama
        if ("tikla" in msg_norm
                or ("butonuna" in msg_norm and "bas" in msg_norm)
                or ("dugmesine" in msg_norm and "bas" in msg_norm)
                or ("yazisina" in msg_norm and ("bas" in msg_norm or "tikla" in msg_norm))):
            return "click_on_screen"

        # Ekrana yazma (not defteri DEĞİL)
        if (("ekrana" in msg_norm or "klavyeden" in msg_norm or "klavye ile" in msg_norm)
                and "yaz" in msg_norm
                and "not defter" not in msg_norm and "notepad" not in msg_norm):
            return "type_on_screen"

        # Kaydırma
        if any(p in msg_norm for p in ("kaydir", "scroll", "asagi in", "yukari cik",
                                       "asagi kay", "yukari kay")):
            return "scroll_screen"

        # Tuş / kısayol
        if ("kisayol" in msg_norm or "ctrl" in words or "alt tab" in msg_norm
                or ("tus" in msg_norm and "bas" in msg_norm)
                or any(k in words for k in ("enter", "esc", "escape", "f5", "f11"))
                or any(t.startswith(("entera", "enter'", "esce", "esc'")) for t in words)):
            return "press_keys"

        return None

    def _extract_click_target(self, message: str) -> dict:
        """'kaydet'e tıkla' → {'target': 'kaydet', 'double': False}."""
        text = (message or "").strip()
        n = self._normalize_turkish(text.lower())
        double = ("cift" in n) or ("double" in n)
        cleaned = text
        cleaned = re.sub(r"\b(çift|cift|double)\b", " ", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(
            r"\b(butonuna|düğmesine|dugmesine|yazısına|yazisina|linkine|sekmesine|simgesine|ikonuna)\b",
            " ", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(
            r"\b(tıkla[yn]?[ıi]?n?|tikla[yn]?[ıi]?n?|bas|üzerine|uzerine|lütfen|lutfen|ekranda|ekran|bir)\b",
            " ", cleaned, flags=re.IGNORECASE)
        cleaned = cleaned.replace("'", " ").replace("’", " ")
        cleaned = re.sub(r"\s+", " ", cleaned).strip(" :,.-\"'")
        # Sonda kalan tek harfli Türkçe yönelme ekini at ("kaydet e" → "kaydet")
        cleaned = re.sub(r"\s+([eaıiuü]|ye|ya|na|ne)$", "", cleaned, flags=re.IGNORECASE).strip()
        if not cleaned:
            cleaned = text
        return {"target": cleaned, "double": double}

    def _extract_screen_type_text(self, message: str) -> str:
        """'ekrana merhaba yaz' → 'merhaba'."""
        text = (message or "").strip()
        cleaned = re.sub(r"^\s*(ekrana|klavyeden|klavye\s*ile|klavyede)\s*",
                         "", text, flags=re.IGNORECASE)
        cleaned = re.sub(r"\s*(diye\s*)?yaz(ar\s*m[ıi]s[ıi]n)?\s*[\.!\?]*$",
                         "", cleaned, flags=re.IGNORECASE)
        return cleaned.strip(" :,-\"'") or text

    def _extract_keys(self, message: str) -> str:
        """Mesajdan tuş/kısayol çıkar: 'ctrl s yap' → 'ctrl+s', 'entera bas' → 'enter'."""
        n = self._normalize_turkish((message or "").lower())
        combos = ("ctrl+shift+esc", "ctrl+s", "ctrl+c", "ctrl+v", "ctrl+z",
                  "ctrl+a", "ctrl+x", "alt+tab", "alt+f4")
        for combo in combos:
            if combo in n or combo.replace("+", " ") in n:
                return combo
        m = re.search(r"\bctrl\s+([a-z])\b", n)
        if m:
            return f"ctrl+{m.group(1)}"
        for key in ("enter", "escape", "esc", "tab", "f5", "f11", "space",
                    "delete", "backspace", "home", "end"):
            if key in n.split() or re.search(rf"\b{key}['’ ]?[aeiy]?\b", n):
    ...
    def _detect_intent_fast(self, user_message: str) -> "str | None":
        ...
        # Not Defteri'ne yazma — "not defterine X yaz", "notepada X ekle"
        # ("aç" varsa ve "yaz/ekle" yoksa bu dalı atla → open_app çalışsın)
        if (("not defter" in msg_norm or "notepad" in msg_norm or "deftere" in msg_norm)
                and any(w in msg_norm for w in ("yaz", "ekle", "not al", "not et"))):
            return "write_to_notepad"

        # Ekran izleme & müdahale komutları (Ollama olmadan da çalışır)
        screen_intent = self._detect_screen_intent(msg_norm)
        if screen_intent:
    ...
    def process_stream(self, user_message: str):
        ...
        if tool_call:
            ...
            final_resp = llm.chat(messages2, temperature=0.1)
    ...
    def _run_llm_loop(self, user_message: str) -> BrainResponse:
        ...
        # İlk LLM çağrısı
        messages = self._context.build_messages()
        llm_response = llm.chat(messages, temperature=0.1)
