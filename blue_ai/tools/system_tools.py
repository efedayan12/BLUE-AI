...
class WeatherTool(BaseTool):
    ...
    # Uluslararası büyük şehirler (Türkiye eki eklenmez)
    _INTL_CITIES = frozenset({
        "london", "paris", "berlin", "madrid", "rome", "moscow", "tokyo",
        "dubai", "new york", "amsterdam", "vienna", "new delhi", "beijing",
    })

    def _normalize_city(self, city: str) -> tuple[str, str]:
        """Şehri normalize et, wttr.in sorgu stringi ve gösterim adını döndür.

        Türkçe şehirler için '+Turkey' eklenir — küçük ilçelerde yanlış
        geocoding sorununu önler (örn. Gölhisar → Evciler sorunu).
        """
        display = city.strip().title()   # Kullanıcıya gösterilecek isim (orijinal)

        # Türkçe karakterleri ASCII'ye çevir (URL için)
        ascii_city = (city.strip()
        ...
        # Uluslararası şehir değilse Turkey ekle — küçük Türk ilçelerini sabitle
        city_lower = ascii_city.lower()
        if city_lower not in self._INTL_CITIES and "," not in ascii_city:
            query = f"{ascii_city}+Turkey"
        else:
            query = ascii_city
    ...
    def execute(self, city: str = "Istanbul", **kwargs) -> ToolResult:
        ...
        query, display_city = self._normalize_city(city)
        ...
        try:
            ...
            url = f"https://wttr.in/{query}?format=j1&lang=tr"
            ...
            return ToolResult(
                ...
                data={"city": display_city, "temp": temp_c, "desc": desc}
            )
        except Exception as e:
...
class WriteNotepadTool(BaseTool):
    """Söyleneni Not Defteri'ne (Notepad) yazar ve açar."""

    name = "write_to_notepad"
    description = "Verilen metni Not Defteri'ne (Notepad) yazar ve açar. Söyleneni nota ekler."
    description_en = "Writes the given text into Notepad and opens it."
    permission_level = PermissionLevel.NOTIFY
    parameters = [
        ToolParameter(
            name="text", type="string",
            description="Not defterine yazılacak metin", required=True,
        ),
        ToolParameter(
            name="mode", type="string",
            description="'ekle' mevcut nota ekler, 'yeni' temiz dosya açar",
            required=False, enum=["ekle", "yeni"], default="ekle",
        ),
    ]

    def execute(self, text: str = "", mode: str = "ekle", **kwargs) -> ToolResult:
        from blue_ai.tools.app_control import write_to_notepad
        text = (text or "").strip()
        if not text:
            return ToolResult(success=False, error="Not defterine yazılacak metin boş.")
        res = write_to_notepad(text, append=(mode != "yeni"))
        if res.get("success"):
            return ToolResult(
                success=True,
                message=res.get("message", "Not defterine yazıldı."),
                data={"path": res.get("path", "")},
            )
        return ToolResult(success=False, error=res.get("error", "Yazılamadı."))


class OpenPathTool(BaseTool):
    """Bir dosyayı veya klasörü açar."""

    name = "open_path"
    description = "Tam yolu verilen bir dosyayı veya klasörü varsayılan uygulamasıyla açar."
    description_en = "Opens a file or folder by its full path."
    permission_level = PermissionLevel.NOTIFY
    parameters = [
        ToolParameter(
            name="path", type="string",
            description="Açılacak dosya veya klasörün tam yolu", required=True,
        ),
    ]

    def execute(self, path: str = "", **kwargs) -> ToolResult:
        from blue_ai.tools.app_control import open_path
        res = open_path(path)
        if res.get("success"):
            return ToolResult(success=True, message=res.get("message", "Açıldı."))
        return ToolResult(success=False, error=res.get("error", "Açılamadı."))


# ═══════════════════════════════════════════════════
#  EKRAN İZLEME ve MÜDAHALE (Screen Vision & Control)
# ═══════════════════════════════════════════════════
class AnalyzeScreenTool(BaseTool):
    """Ekranda ne olduğunu okur ve özetler (aktif pencere + OCR metni)."""

    name = "analyze_screen"
    description = "Ekrana bakar; aktif pencereyi ve ekrandaki yazıları okuyup özetler."
    description_en = "Reads the screen: active window + on-screen text (OCR) summary."
    permission_level = PermissionLevel.AUTO
    parameters = []

    def execute(self, **kwargs) -> ToolResult:
        from blue_ai.tools.screen import describe_screen
        d = describe_screen()
        return ToolResult(success=True, message=d["summary"],
                          data={"active_window": d.get("active_window"),
                                "word_count": d.get("word_count", 0)})


class ClickOnScreenTool(BaseTool):
    """Ekranda yazılı bir öğeyi bulup tıklar (OCR ile konum bulma)."""

    name = "click_on_screen"
    description = "Ekranda yazan bir metni/butonu bulup üzerine tıklar (ör: 'Kaydet')."
    description_en = "Finds visible text/button on screen and clicks it."
    permission_level = PermissionLevel.NOTIFY
    parameters = [
        ToolParameter(name="target", type="string",
                      description="Tıklanacak ekran metni/butonu (ör: Kaydet, Dosya)", required=True),
        ToolParameter(name="double", type="boolean",
                      description="Çift tıklama yapılsın mı", required=False, default=False),
    ]

    def execute(self, target: str = "", double: bool = False, **kwargs) -> ToolResult:
        from blue_ai.tools.screen import click_text
        target = (target or "").strip()
        if not target:
            return ToolResult(success=False, error="Tıklanacak hedef belirtilmedi.")
        res = click_text(target, double=bool(double))
        if res.get("success"):
            return ToolResult(success=True, message=res["message"])
        err = res.get("error", "Bulunamadı.")
        cands = res.get("candidates") or []
        if cands:
            err += " Ekranda görünenler: " + ", ".join(cands[:6])
        return ToolResult(success=False, error=err)


class TypeOnScreenTool(BaseTool):
    """Odaklı pencereye/alana klavyeden metin yazar."""

    name = "type_on_screen"
    description = "Aktif (odaklı) pencereye verilen metni klavyeden yazar."
    description_en = "Types the given text into the focused window via keyboard."
    permission_level = PermissionLevel.NOTIFY
    parameters = [
        ToolParameter(name="text", type="string",
                      description="Yazılacak metin", required=True),
    ]

    def execute(self, text: str = "", **kwargs) -> ToolResult:
        from blue_ai.tools.screen import type_text
        text = text or ""
        if not text.strip():
            return ToolResult(success=False, error="Yazılacak metin boş.")
        res = type_text(text)
        return (ToolResult(success=True, message=res["message"]) if res.get("success")
                else ToolResult(success=False, error=res.get("error", "Yazılamadı.")))


class PressKeysTool(BaseTool):
    """Klavye tuşuna veya kısayoluna basar (enter, esc, ctrl+s ...)."""

    name = "press_keys"
    description = "Klavye tuşu/kısayolu gönderir. Tek tuş ('enter') veya kombinasyon ('ctrl+s')."
    description_en = "Presses a key or shortcut (e.g. 'enter', 'ctrl+s')."
    permission_level = PermissionLevel.NOTIFY
    parameters = [
        ToolParameter(name="keys", type="string",
                      description="Tuş veya kısayol, ör: enter, esc, ctrl+s, alt+tab", required=True),
    ]

    def execute(self, keys: str = "", **kwargs) -> ToolResult:
        from blue_ai.tools.screen import press_key, hotkey
        keys = (keys or "").strip().lower().replace(" ", "")
        if not keys:
            return ToolResult(success=False, error="Tuş belirtilmedi.")
        parts = [p for p in keys.replace("-", "+").split("+") if p]
        res = hotkey(*parts) if len(parts) > 1 else press_key(parts[0])
        return (ToolResult(success=True, message=res["message"]) if res.get("success")
                else ToolResult(success=False, error=res.get("error", "Olmadı.")))


class ScrollScreenTool(BaseTool):
    """Ekranı yukarı/aşağı kaydırır."""

    name = "scroll_screen"
    description = "Fare tekerini kullanarak ekranı yukarı veya aşağı kaydırır."
    description_en = "Scrolls the screen up or down."
    permission_level = PermissionLevel.AUTO
    parameters = [
        ToolParameter(name="direction", type="string",
                      description="'asagi' veya 'yukari'", required=False,
                      enum=["asagi", "yukari"], default="asagi"),
        ToolParameter(name="amount", type="integer",
                      description="Kaydırma adımı (varsayılan 3)", required=False, default=3),
    ]

    def execute(self, direction: str = "asagi", amount: int = 3, **kwargs) -> ToolResult:
        from blue_ai.tools.screen import scroll
        try:
            amt = int(amount)
        except Exception:
            amt = 3
        step = -abs(amt) if str(direction).lower().startswith(("asa", "down", "alt")) else abs(amt)
        res = scroll(step)
        return (ToolResult(success=True, message=res["message"]) if res.get("success")
                else ToolResult(success=False, error=res.get("error", "Olmadı.")))
...
class WatchScreenTool(BaseTool):
    """Canlı ekran izlemeyi başlatır/durdurur."""

    name = "watch_screen"
    description = "Canlı ekran izlemeyi başlatır veya durdurur (panelde canlı görüntü)."
    description_en = "Starts or stops live screen watching."
    permission_level = PermissionLevel.AUTO
    parameters = [
        ToolParameter(name="action", type="string",
                      description="'baslat' veya 'durdur'", required=False,
                      enum=["baslat", "durdur"], default="baslat"),
    ]

    def execute(self, action: str = "baslat", **kwargs) -> ToolResult:
        from blue_ai.tools.screen import get_watcher
        w = get_watcher()
        act = (action or "baslat").lower()
        if act.startswith("dur") or act in ("stop", "kapat"):
            w.stop()
            return ToolResult(success=True, message="Canlı ekran izleme durduruldu. 🛑")
        started = w.start()
        msg = ("Canlı ekran izleme başladı — ekranını izliyorum. 👁️" if started
               else "Zaten ekranını izliyorum. 👁️")
